...
const SignUpForm: React.FC = () => {
		...
		if (password === "") {
			...
			setErrors((prevErrors) => {
				return {
					...prevErrors,
					password:
						"Password must be atleast 8 characters long and includes atleast 1 lowercase character, 1 uppercase characters, 1 number, and 1 special character",
				}
			})
		} else {
			setErrors((prevErrors) => {
	...
	const signUp = () => {
		const data: Data = {
			...
			isValidCfmPassword: boolean = validateCfmPassword(
				data.password,
				data.cfmPassword
			),
			isValidData: boolean =
	...
	return (
		<div className="sign-up-form flex flex-col items-center w-full gap-y-7 mt-16">
			...
			<div className="w-[35%]">
				...
				/>
				{errors.cfmPassword !== "" && (
					<p className="error">{errors.cfmPassword}</p>
				)}
			</div>

			<button
				className="btn-secondary w-[220px] h-[50px]"
				onClick={signUp}
			>
				sign up
			</button>
			...
			<p className="text-white">
				...
				<Link to="/login" className="text-[#D7BDF2]">
					{" "}
					Login.
				</Link>
			</p>
